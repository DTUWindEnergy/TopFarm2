...
if not importlib.util.find_spec("topfarm"):
    ...
    def check_code(self):
        ...
        lines = [fix(x) for x in code.split("\n")]

        # import * only allowed at module level
        # So extract and remove from code lines
        module_imports = [x for x in lines if x.startswith('from') and x.endswith('import *')]
        ...
        try:
            ...
            with contextlib.redirect_stdout(StringIO()):
                with contextlib.redirect_stderr(StringIO()):
                    # execute module level imports (stored in x dict) and use a locals in execution of code
                    g, x = {}, {}
                    exec("\n".join(module_imports), g, x)
...
It has now been auto insert. Please check the notebook and commit the changes""" % os.path.abspath(self.filename))
    ...
    def remove_empty_end_cell(self):
        while self.cells[-1]['cell_type'] == 'code' and all([x.strip() == "" for x in self.cells[-1]['source']]):
...
if __name__ == '__main__':
    import topfarm
    nb = Notebook(os.path.dirname(topfarm.__file__) + '/../docs/notebooks/constraints.ipynb')
    nb.check_code()
    nb.check_links()
