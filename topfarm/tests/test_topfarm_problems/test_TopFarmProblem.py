...
from topfarm._topfarm import TurbineTypeOptimizationProblem, ProblemComponent
...
def test_state(turbineTypeOptimizationProblem):
    tf = turbineTypeOptimizationProblem
    npt.assert_equal(tf.state, {'turbineType': [0, 0, 0]})


def test_state_array(turbineTypeOptimizationProblem):
    tf = turbineTypeOptimizationProblem
    arr = tf.state_array(['turbineType', 'turbineType'])
...
def test_evaluate(turbineTypeOptimizationProblem):
    ...
    assert cost == 5
    np.testing.assert_array_equal(state['turbineType'], [0, 0, 0])
...
def cost(turbineX, turbineY):
    return np.sum((turbineX - optimal[:, 0])**2 + (turbineY - optimal[:, 1])**2)


def gradients(turbineX, turbineY):
    return ((2 * turbineX - 2 * optimal[:, 0]),
            (2 * turbineY - 2 * optimal[:, 1]))


def wrong_gradients(turbineX, turbineY):
    return ((2 * turbineX - 2 * optimal[:, 0]) + 1,
            (2 * turbineY - 2 * optimal[:, 1]))
