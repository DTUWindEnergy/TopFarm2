...
def main():
    if __name__ == '__main__':
        ...
        cost_comp2 = CostModelComponent(input_keys=[('x', x_init),('y', y_init)],
                                                  n_wt=n_wt,
                                                  cost_function=aep_func,
                                                  objective=True,
                                                  maximize=True,
        ...
        if 1:
            ...
            plt.figure()
            plt.plot(recorder2['water_depth'].min((1)))
            plt.plot([0,recorder2['water_depth'].shape[0]],[maximum_water_depth, maximum_water_depth])
            plt.xlabel('Iteration')
            plt.ylabel('Max depth [m]')
            plt.show()
        ...
        #     ax2.plot(line[:, 0], line[:,1], 'r')
        ax2.plot(xs, ys)
        problem2.model.plot_comp.plot_current_position(state2['x'], state2['y'])
        ax2.set_title(f'Max Water Depth Boundary: {maximum_water_depth} m')
        
        
        cost_comp3 = CostModelComponent(input_keys=[('x', x_init),('y', y_init)],
                                                  n_wt=n_wt,
                                                  cost_function=aep_func,
                                                  objective=True,
                                                  maximize=True,
        ...
        problem3 = TopFarmProblem(design_vars={'x': x_init, 'y': y_init},
                                constraints=[XYBoundaryConstraint([(boundary, 1), (np.asarray((xs,ys)).T, 0)], boundary_type='multi_polygon'),
                                              SpacingConstraint(min_spacing)],
                                    # post_constraints=[('water_depth', {'lower': np.ones(n_wt) * maximum_water_depth})],
                                  cost_comp=cost_comp3,
                                    driver=EasyScipyOptimizeDriver(optimizer='SLSQP', maxiter=maxiter, tol=tol),
                                    # driver=EasyRandomSearchDriver(RandomizeTurbinePosition()),
                                  plot_comp=XYPlotComp(),
                                  expected_cost=ec)
        ...
        if 1:
            ...
            plt.figure()
            plt.plot(recorder3['water_depth'].min((1)))
            plt.plot([0,recorder3['water_depth'].shape[0]],[maximum_water_depth, maximum_water_depth])
            plt.xlabel('Iteration')
            plt.ylabel('Max depth [m]')
            plt.show()
        ...
        #     ax2.plot(line[:, 0], line[:,1], 'r')
        ax2.plot(xs, ys)
        problem3.model.plot_comp.plot_current_position(state3['x'], state3['y'])
        ax2.set_title(f'Max Water Depth Boundary: {maximum_water_depth} m')
