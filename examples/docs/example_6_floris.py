    ...
    if __name__ == '__main__':
        if not 'plantenergy' in topfarm.plugins:
            ...
            def cost_func(AEP, **kwargs):
                return AEP
            cost_comp = CostModelComponent(input_keys=[('AEP', [0])], n_wt=n_wt, cost_function=cost_func, output_key="aep", output_unit="kWh", objective=True, income_model=True, input_units=['kW*h'])
            ...
            #        driver=EasyScipyOptimizeDriver(optimizer='COBYLA'),
                    constraints=[SpacingConstraint(200,'m'),
            ...
            #        driver=EasyScipyOptimizeDriver(optimizer='COBYLA'),
                    ...
                    plot_comp=plot_comp,
                    expected_cost=-100e2)
            prob2['turbineX'] = turbineX
            prob2['turbineY'] = turbineY
            prob2['hubHeight'] = turbineZ
            prob2['yaw0'] = yaw
            prob2['rotorDiameter'] = rotorDiameter
            prob2['hubHeight'] = hubHeight
            prob2['axialInduction'] = axialInduction
            prob2['generatorEfficiency'] = generatorEfficiency
            prob2['windSpeeds'] = np.ones(len(f))*wind_speed
            prob2['air_density'] = air_density
            
            prob2['windDirections'] = np.arange(0,360,360/len(f))
            prob2['windFrequencies'] = f/100
            prob2['model_params:cos_spread'] = 1E12         # turns off cosine spread (just needs to be very large)
            prob2['model_params:shearExp'] = 0.25      
            prob2['model_params:z_ref'] = 80.         
            prob2['model_params:z0'] = 0.        
            prob2['rated_power'] = np.ones(n_wt)*5000.
            prob2['cut_in_speed'] = np.ones(n_wt)*3
            prob2['cp_curve_wind_speed'] = cp_curve[:,0]
            prob2['cp_curve_cp'] = cp_curve[:,1]
            prob2['rated_wind_speed'] = np.ones(n_wt)*11.4
            prob2['cut_out_speed'] = np.ones(n_wt) * 25.0
            cost_init, state_init = prob2.evaluate()
            #if 0:
            #prob2.check_partials(compact_print=True,includes='*direction_group0*')
            #else:
            tic = time.time()
